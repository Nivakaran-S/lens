...
/**
 * Append a payment row to the audit log. The unique-sparse index on
 * stripe_session_id makes this idempotent for Stripe webhooks: a duplicate
 * delivery attempts to insert with the same session id and Mongo throws
 * E11000, which the caller can catch and treat as "already credited".
 */
...
export async function paymentForSession(stripeSessionId: string): Promise<PaymentDoc | null> {
  const payments = await paymentsCollection();
  const doc = await payments.findOne({ stripe_session_id: stripeSessionId });
  if (!doc) return null;
